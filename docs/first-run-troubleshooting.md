...
| --- | --- | --- |
| `Owner mapping file was not found` | `-OwnerMappingPath` was passed, but the CSV does not exist at that path. | Remove `-OwnerMappingPath` for the first run, or create `owner-mapping.csv` with `Pattern`, `Owner`, and `BusinessUnit`. The startup wizard checks the `inputs` folder for `owner-mapping.csv`, `ownership-enrichment.csv`, ownership context graph files, and `discounted-principals.csv`; prefer that flow or the splatted examples because they only pass optional paths when files exist. If `owner-mapping.csv` is missing, interactive startup can queue a post-scan `owner-mapping-draft.csv` so the first validated export gives you a starter mapping to fill. |
| `ownership-enrichment.csv` is missing | You have HR, employee, OBS, project, or owner CSVs, but no ShareSurfer enrichment file yet. | Run `Start-ShareSurfer.ps1` interactively and accept the ownership enrichment setup prompt, or run `Join-ShareSurferOwnershipSources -Interactive -BrowseForCsv -IncludeContextGraph` manually. Keep the generated `ownership-import.definition.json` and `ownership-import-rerun.ps1` so the same import can be rebuilt without repeating the interview. |
| Ownership import appears stuck with no new messages | Older builds could spend a long time merging large CSVs or enriching rows from AD without printing a heartbeat. | If no output has appeared for a long time, press `Ctrl+C`, upgrade to a build with ownership import progress, and rerun from `ownership-import.definition.json` or `ownership-enrichment-rerun.ps1`. The import is read-only against CSVs and AD and only writes local output files. |
...
| `Test-Path "$releaseRoot\src\ShareSurfer\ShareSurfer.psd1"` returns `False` | The release ZIP may have been extracted into a doubled folder such as `C:\ShareSurfer-0.1.0-pre.38\ShareSurfer-0.1.0-pre.38`. | Move the inner release folder up one level, or extract the ZIP again to `C:\` so `$releaseRoot` points at `C:\ShareSurfer-0.1.0-pre.38`. |
...
- Native SMB/RPC descriptor errors show that the share was reachable but owner, DACL, or share-permission evidence was unreadable or unparseable.
