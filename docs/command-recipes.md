...
Use this on the Windows collector host after downloading `ShareSurfer-0.1.0-pre.11.zip` from the GitHub release on an approved connected workstation.
...
Both `Test-Path` commands should return `True`. If either returns `False`, check for a doubled folder such as `C:\ShareSurfer\ShareSurfer-0.1.0-pre.11\ShareSurfer-0.1.0-pre.11`.
