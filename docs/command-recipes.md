...
If `v0.1.0-pre.23` is not visible yet on the [ShareSurfer Releases page](https://github.com/jonathanweinberg/ShareSurfer/releases), use the latest published prerelease and substitute that version in every `ShareSurfer-0.1.0-pre.23` path and ZIP name below. The commands also assume Windows PowerShell 5.1 unless a command explicitly says otherwise.
...
Use this on the Windows collector host after downloading `ShareSurfer-0.1.0-pre.23.zip` from the GitHub release on an approved connected workstation. If that checkpoint ZIP is not published yet, download the latest published prerelease ZIP and update `$releaseZip` and `$releaseRoot` to match it.
...
Both `Test-Path` commands should return `True`. If either returns `False`, check for a doubled folder such as `C:\ShareSurfer-0.1.0-pre.23\ShareSurfer-0.1.0-pre.23`.
