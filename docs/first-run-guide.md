...
1. Extract the current `v0.1.0-pre.39` release ZIP to `C:\`. If that checkpoint tag is not visible on the [ShareSurfer Releases page](https://github.com/jonathanweinberg/ShareSurfer/releases) yet, use the latest published prerelease and substitute that version in the paths below.
2. Use `C:\ShareSurfer-0.1.0-pre.39\` as `$releaseRoot`, or replace the version folder with the published prerelease you actually extracted.
...
If you are using the `v0.1.0-pre.39` release ZIP, extract it to `C:\`. If that checkpoint tag is not visible yet, use the latest published prerelease and substitute that version in the paths below. The extracted release root should be:

```text
C:\ShareSurfer-0.1.0-pre.39\
```

If Windows Explorer suggests extracting to `C:\ShareSurfer-0.1.0-pre.39`, change the destination to `C:\` so you do not end up with a doubled nested folder. From PowerShell:

```powershell
$releaseZip = 'C:\Downloads\ShareSurfer-0.1.0-pre.39.zip'
$releaseRoot = 'C:\ShareSurfer-0.1.0-pre.39'
...
The collector prints timestamped status lines while it runs. That is expected and helps first-time operators tell the scan is still active during recursive folder enumeration, ACL reads, identity enrichment, and CSV export. Long identity work now reports root identity counts, processed identity progress, group expansion progress, directory lookup attempts/cache hits, and potential service-account counts. Export work reports each classification stage before and after conflicts, findings, permissioned groups, owner/business-unit pivots, migration discovery, owner review packets, evidence confidence, and CSV writing. When the scan finishes, the `ShareSurfer Summary` lines show counts for shares, items, findings, conflicts, collection errors, and partial shares. They also show the output path and the next `Test-ShareSurferExport` command to run. If the summary mentions collection errors or partial shares, open `collection_errors.csv` and the Diagnostics view before asking an owner to approve the result. Add `-Quiet` when a scheduled or scripted run should suppress console progress.
