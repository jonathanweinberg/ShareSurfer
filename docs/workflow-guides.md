...
1. Extract the current release ZIP to `C:\` and use `C:\ShareSurfer-0.1.0-pre.20\` as the release root. If `v0.1.0-pre.20` is not visible yet on the [ShareSurfer Releases page](https://github.com/jonathanweinberg/ShareSurfer/releases), use the latest published prerelease and substitute that version in the example paths.
