...
If Windows Explorer suggests extracting to `C:\ShareSurfer\ShareSurfer-0.1.0-pre.8`, change the destination to `C:\ShareSurfer` to avoid a doubled folder such as `C:\ShareSurfer\ShareSurfer-0.1.0-pre.8\ShareSurfer-0.1.0-pre.8`. From PowerShell, use:
...
Use this path when the collector host cannot use internet access, npm, browser tooling, or a dashboard preview server. Prefer the [v0.1.0-pre.8 release zip](https://github.com/jonathanweinberg/ShareSurfer/releases/tag/v0.1.0-pre.8) for first-time Windows use because it already includes the built dashboard assets. Copy the unpacked ShareSurfer release folder to the collector host first. If the ZIP is extracted to `C:\ShareSurfer\`, use `C:\ShareSurfer\ShareSurfer-0.1.0-pre.8` as `$shareSurferRoot`. The collector only needs PowerShell 5.1, read access to the target share, and directory read access for identity enrichment.
