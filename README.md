...
### Ownership Import and Reusable Commands

![Ownership import and reusable commands workflow](docs/visuals/readme-flow-guides/ownership-import-reusable-commands.png)

Use this flow when an HR, employee, OBS, OID, cost-center, project, or owner CSV has useful data but unexpected headers. ShareSurfer tests each source, saves reusable mapping profiles, writes `normalized-ownership.csv` for single-file cleanup, and can join multiple source CSVs into a pre-scan enrichment file. For the first multi-CSV run, `Join-ShareSurferOwnershipSources -Interactive -BrowseForCsv -DefinitionPath ...` opens a text-only picker so admins can browse folders and choose CSV files without a GUI. The saved definition JSON remembers the selected CSV paths and settings for later reruns. Pass the enriched CSV to `Invoke-ShareSurferScan -OwnershipEnrichmentPath`; the scan exports it as `ownership_enrichment.csv` so the standalone dashboard can show matched, ambiguous, source-only, forbidden-OU-skipped, and potential service-account rows beside the rest of the evidence.
...
| You have an HR, employee, OBS, OID, or owner CSV with unexpected headers | Use the [admin ownership import guide](docs/admin-ownership-import.md). |
| You need a copy/paste HR or OBS CSV ingest walkthrough | Use the [ownership CSV ingest quick reference](docs/ownership-csv-ingest-quick-reference.md). |
...
Current pre-release quickstart package: [v0.1.0-pre.16](https://github.com/jonathanweinberg/ShareSurfer/releases/tag/v0.1.0-pre.16). Download `ShareSurfer-0.1.0-pre.16.zip` and `ShareSurfer-0.1.0-pre.16.zip.sha256` from that release on an approved connected workstation, verify or record the SHA256 value, then move the zip by your normal approved process. The package is unsigned, but it is fully built and includes the PowerShell module, scripts, documentation, release manifest, dependency-age report, SHA256 files, and prebuilt standalone dashboard template assets.

When the ZIP is extracted to `C:\ShareSurfer\`, the release root is:

```text
C:\ShareSurfer\ShareSurfer-0.1.0-pre.16\
```

If Windows Explorer suggests extracting to `C:\ShareSurfer\ShareSurfer-0.1.0-pre.16`, change the destination to `C:\ShareSurfer` to avoid a doubled folder such as `C:\ShareSurfer\ShareSurfer-0.1.0-pre.16\ShareSurfer-0.1.0-pre.16`. From PowerShell, use:

```powershell
$releaseZip = 'C:\ShareSurfer\downloads\ShareSurfer-0.1.0-pre.16.zip'
...
If you have an HR, employee, OBS, OID, project, or owner CSV with unexpected headers, use `Test-ShareSurferOwnershipSource`, `New-ShareSurferOwnershipMappingProfile`, and `Import-ShareSurferOwnershipSource` before the scan. These commands inspect the headers, create a reusable mapping profile, and normalize one source into `normalized-ownership.csv` without internet access or uploading the file. When ownership facts are split across multiple CSVs, run `Join-ShareSurferOwnershipSources -Interactive -BrowseForCsv` before the scan to pick CSVs in a text console, save the selected paths and settings to `ownership-import.definition.json`, and create `ownership-enrichment.csv`. Later reruns can use `Join-ShareSurferOwnershipSources -DefinitionPath 'C:\ShareSurfer\inputs\ownership-import.definition.json' -OutputPath 'C:\ShareSurfer\inputs\ownership-enrichment.csv' -Force` without repeating the picker choices. Pass the enriched CSV with `Invoke-ShareSurferScan -OwnershipEnrichmentPath`; the scan exports it as `ownership_enrichment.csv`, and the standalone dashboard uses that exported dataset after the export package is generated. See the [admin ownership import guide](docs/admin-ownership-import.md), or send the shorter [ownership CSV ingest quick reference](docs/ownership-csv-ingest-quick-reference.md) to teams that just need copy/paste commands.

Typical ownership import outputs are:

| Output | What It Is For |
| --- | --- |
| `C:\ShareSurfer\inputs\hr-obs.mapping.json` | Saved header map so the admin does not repeat the header interview. |
| `C:\ShareSurfer\inputs\normalized-ownership.csv` | Canonical ownership data from one source file, with employee, manager, OBS, title, office, owner, and review-warning fields. |
| `C:\ShareSurfer\inputs\ownership-import-rerun.ps1` | Reusable commands to test the same source and regenerate `normalized-ownership.csv`. |
| `C:\ShareSurfer\inputs\ownership-import.definition.json` | Reusable multi-CSV join definition with selected CSV paths and settings. This is not scan evidence by itself. |
| `C:\ShareSurfer\inputs\ownership-enrichment.csv` | Pre-scan joined ownership input from one or more CSVs plus optional AD enrichment. |
| `$exportPath\ownership_enrichment.csv` | Exported scan evidence used by the standalone dashboard for ownership enrichment review. |
| `C:\ShareSurfer\inputs\owner-mapping-draft.csv` | Admin-fillable path-to-owner starter file for unmapped shares or top folders. |
| `C:\ShareSurfer\inputs\owner-mapping-rerun.ps1` | Reusable commands to regenerate the owner mapping draft and use the completed `owner-mapping.csv` on the next scan. |

If you do not know owners yet, run a scan first and then create a draft owner mapping for an admin to fill in:
...
Use this path when the collector host cannot use internet access, npm, browser tooling, or a dashboard preview server. Prefer the [v0.1.0-pre.16 release zip](https://github.com/jonathanweinberg/ShareSurfer/releases/tag/v0.1.0-pre.16) for first-time Windows use because it already includes the built dashboard assets. Copy the unpacked ShareSurfer release folder to the collector host first. If the ZIP is extracted to `C:\ShareSurfer\`, use `C:\ShareSurfer\ShareSurfer-0.1.0-pre.16` as `$shareSurferRoot`. The collector only needs PowerShell 5.1, read access to the target share, and directory read access for identity enrichment.
...
Release packaging enforces the dependency-age policy recorded in `release-metadata.json`; the current prerelease requires npm package versions to be at least 7 days old before they are included in a release package. This helps avoid pulling a just-published dependency into a pre-release. Local dry runs can use `-SkipDependencyAgeCheck`, but tagged release packaging should keep the check enabled.
