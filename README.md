...
After reviewers make owner or migration decisions, keep that decision evidence with the export:

```powershell
$decisionPath = 'C:\ShareSurfer\reviews\scan-001'
...
# Reviewers edit owner_review_decisions.csv and migration_cluster_decisions.csv.
...
Allowed decision values are `ConfirmedOwner`, `CleanupNeeded`, `RerunNeeded`, `MigrationCandidate`, and `WrongOwner`. The rerun file lets the same local CSV workflow be repeated after a scan rerun without rebuilding the commands.
