...
Current pre-release quickstart package: [v0.1.0-pre.15](https://github.com/jonathanweinberg/ShareSurfer/releases/tag/v0.1.0-pre.15). Download `ShareSurfer-0.1.0-pre.15.zip` and `ShareSurfer-0.1.0-pre.15.zip.sha256` from that release on an approved connected workstation, verify or record the SHA256 value, then move the zip by your normal approved process. The package is unsigned, but it is fully built and includes the PowerShell module, scripts, documentation, release manifest, dependency-age report, SHA256 files, and prebuilt standalone dashboard template assets.

When the ZIP is extracted to `C:\ShareSurfer\`, the release root is:

```text
C:\ShareSurfer\ShareSurfer-0.1.0-pre.15\
```

If Windows Explorer suggests extracting to `C:\ShareSurfer\ShareSurfer-0.1.0-pre.15`, change the destination to `C:\ShareSurfer` to avoid a doubled folder such as `C:\ShareSurfer\ShareSurfer-0.1.0-pre.15\ShareSurfer-0.1.0-pre.15`. From PowerShell, use:

```powershell
$releaseZip = 'C:\ShareSurfer\downloads\ShareSurfer-0.1.0-pre.15.zip'
$releaseRoot = 'C:\ShareSurfer\ShareSurfer-0.1.0-pre.15'
...
Use this path when the collector host cannot use internet access, npm, browser tooling, or a dashboard preview server. Prefer the [v0.1.0-pre.15 release zip](https://github.com/jonathanweinberg/ShareSurfer/releases/tag/v0.1.0-pre.15) for first-time Windows use because it already includes the built dashboard assets. Copy the unpacked ShareSurfer release folder to the collector host first. If the ZIP is extracted to `C:\ShareSurfer\`, use `C:\ShareSurfer\ShareSurfer-0.1.0-pre.15` as `$shareSurferRoot`. The collector only needs PowerShell 5.1, read access to the target share, and directory read access for identity enrichment.

```powershell
$shareSurferRoot = 'C:\ShareSurfer\ShareSurfer-0.1.0-pre.15'
$exportPath = 'C:\ShareSurfer\exports\scan-001'
