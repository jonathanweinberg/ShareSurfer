...
| Flexible ownership import | Normalize HR, employee, OBS, OID, or owner CSVs with unexpected headers | `hr-obs.mapping.json`, `normalized-ownership.csv`, `owner-mapping-draft.csv`, and reusable rerun `.ps1` files |
...
Use this flow when an HR, employee, OBS, OID, cost-center, or owner CSV has useful data but unexpected headers. ShareSurfer tests the source, saves a reusable mapping profile, writes `normalized-ownership.csv`, creates `owner-mapping-draft.csv` when scan paths need owner routing, and can write rerun `.ps1` command files so the same import can be repeated without redoing the header interview.
...
If you have an HR, employee, OBS, OID, or owner CSV with unexpected headers, use `Test-ShareSurferOwnershipSource`, `New-ShareSurferOwnershipMappingProfile`, and `Import-ShareSurferOwnershipSource` before the scan. These commands inspect the headers, create a reusable mapping profile, and normalize the source into a canonical ownership CSV without AI/LLM calls or internet access. They also return reusable command text, and can write a `.ps1` file with `-ReusableCommandPath` so the next refresh can reuse the saved profile. See the [admin ownership import guide](docs/admin-ownership-import.md), or send the shorter [ownership CSV ingest quick reference](docs/ownership-csv-ingest-quick-reference.md) to teams that just need copy/paste commands.
...
| `C:\ShareSurfer\inputs\normalized-ownership.csv` | Canonical ownership data with employee, manager, OBS, title, office, owner, and review-warning fields. |
