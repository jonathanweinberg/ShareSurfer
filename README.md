...
If you have an HR, employee, OBS, OID, or owner CSV with unexpected headers, use `Test-ShareSurferOwnershipSource`, `New-ShareSurferOwnershipMappingProfile`, and `Import-ShareSurferOwnershipSource` before the scan. These commands inspect the headers, create a reusable mapping profile, and normalize the source into a canonical ownership CSV without AI/LLM calls or internet access. See the [admin ownership import guide](docs/admin-ownership-import.md).
