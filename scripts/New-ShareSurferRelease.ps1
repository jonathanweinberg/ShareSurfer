...
$releaseNotes = @(
    ...
    '## Highlights',
    '',
    '- Adds the guided startup review/run handoff: after the first-run questions, ShareSurfer shows the generated JSON config, operator plan, and rerun script paths, offers to display them, and asks whether to launch the generated scan/validate/dashboard workflow now.',
    '- Keeps the safer startup default: reviewing generated files is encouraged, and launching the actual scan remains an explicit operator choice.',
    '- Includes the hardened release-root `Start-ShareSurfer.ps1` launcher that recursively checks ShareSurfer PowerShell files and explicitly clears Windows `Zone.Identifier` downloaded-file markers where the platform exposes them.',
    '- Keeps the static standalone dashboard template assets packaged for offline use after a validated export is bundled.',
    '',
    '## Operator Impact',
    '',
    '- First-time Windows operators no longer have to infer the next step after answering startup questions: the wizard can show the generated evidence files and start the generated rerun script from the same session.',
    '- Teams that prefer a review-before-run process can answer **No** at the launch prompt, inspect the saved JSON and rerun script, and reuse the same startup choices later.',
    '- Locked-down and nonpermissive workflows keep the documented recursive `Unblock-File` path, plus startup summary fields that report file unblock status.',
    '',
    '## Package Contents',
    '',
