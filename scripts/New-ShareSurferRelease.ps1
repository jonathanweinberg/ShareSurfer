...
$releaseNotes = @(
    ...
    '## Highlights',
    '',
    '- Hardened the release-root `Start-ShareSurfer.ps1` launcher so it recursively checks ShareSurfer PowerShell files and explicitly clears Windows `Zone.Identifier` downloaded-file markers where the platform exposes them.',
    '- Clarified the no-prompt first-run path: run the documented recursive `Unblock-File` command before launching ShareSurfer. If the launcher is started first, Windows may still ask once for the launcher itself before ShareSurfer can clear the rest of the folder.',
    '- Preserved the guided startup workflow: `Start-ShareSurfer.ps1` imports the module, starts `Start-ShareSurferStartup`, writes reusable JSON choices, and generates the operator plan plus rerun script before any scan is run.',
    '- Keeps the static standalone dashboard template assets packaged for offline use after a validated export is bundled.',
    '',
    '## Operator Impact',
    '',
    '- First-time Windows operators should see fewer one-file-at-a-time PowerShell approval prompts after the launcher starts.',
    '- Locked-down and nonpermissive workflows now have clearer copy/paste unblock guidance in the README, first-run guide, and command recipes.',
    '- The startup summary reports how many downloaded-file markers were explicitly cleared during startup unblock processing.',
    '',
    '## Package Contents',
    '',
