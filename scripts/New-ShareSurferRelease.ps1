...
$releaseNotes = @(
    ...
    '',
    'This is an unsigned pre-1.0 package intended for field validation, operator workflow testing, and early business-owner review pilots.',
    '',
    '## Highlights',
    '',
    '- Opens the staged `Start-ShareSurfer` Start Menu from the release-root `Start-ShareSurfer.ps1` launcher by default.',
    '- Keeps the hardened release-root launcher behavior that recursively checks ShareSurfer PowerShell files and clears Windows `Zone.Identifier` downloaded-file markers where the platform exposes them.',
    '- Keeps saved startup config replay available with `-ConfigPath`, so existing rerun workflows continue to work.',
    '- Shows readiness and command previews for ownership inputs, guided scan setup, export validation, dashboard packaging, stop gates, and support bundle guidance before asking for confirmation.',
    '- Makes ownership import prompts safer: Enter accepts an existing suggestion, while fields without suggestions require a numbered/header choice or explicit `S` skip.',
    '- Keeps the static standalone dashboard template assets packaged for offline use after a validated export is bundled.',
    '',
    '## Operator Impact',
    '',
    '- First-time Windows operators get a task menu instead of having to remember which command comes next.',
    '- Teams can still review generated JSON, plans, and rerun scripts before launching collection; ShareSurfer remains read-only against target shares.',
    '- Guided startup points operators to the generated evidence files, shows the startup JSON/plan/rerun script on request, and keeps scan launch as an explicit operator choice.',
    '- Ownership import is harder to mis-key because blank Enter no longer silently skips unmapped fields.',
    '- Locked-down and nonpermissive workflows keep the documented recursive `Unblock-File` path, startup replay, and offline dashboard package flow.',
    '',
    '## Package Contents',
    '',
    '- PowerShell module under `src/ShareSurfer`.',
    '- Release-root launcher: `Start-ShareSurfer.ps1`.',
    '- Operator scripts under `scripts`.',
    '- Documentation and quickstarts.',
    '- Prebuilt standalone dashboard template assets under `interface/standalone-dashboard/dist`.',
    '- `release-manifest.json`, `SHA256SUMS.txt`, `dependency-age-report.json`, and this `RELEASE.md`.',
    '',
    '## Validation Summary',
    '',
    '- PowerShell parser/import checks are expected before publish.',
    '- ShareSurfer test suite is expected before publish.',
    '- Release packaging validates ZIP contents, manifest, hashes, dashboard assets, and dependency-age policy.',
    '- GitHub Actions should pass Build/test and Windows PowerShell 5.1 smoke before merge.',
    '',
    ('Package notes: {0}' -f [string]$releaseMetadata.releaseNotesSummary),
    '',
    '## Dashboard Notes',
    '',
